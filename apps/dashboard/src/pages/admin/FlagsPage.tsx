...
export function FlagsPage() {
  ...
  const reviewMutation = useMutation({
    mutationFn: async ({
      ...
      const { error } = await api.POST('/api/v1/admin/flags/{id}/review', {
        params: { path: { id } },
        body: { status, reason },
      });
      if (error) throw error;
    ...
    onError: (error) =>
      toast.error("Couldn't update flag", { description: apiErrorMessage(error) }),
  });
  ...
  const columns: ColumnDef<FlagRow, unknown>[] = [
    ...
    { accessorKey: 'kind', header: 'Kind' },
    {
      ...
      header: 'Severity',
      cell: (c) => (
        <Badge tone={severityTone[c.getValue() as string]}>{c.getValue() as string}</Badge>
      ),
    },
    {
      accessorKey: 'status',
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        title="Flags"
        description="Abuse and fraud signals from trial protection and abuse.scan."
      />
      ...
      <div className="flex flex-wrap gap-3">
        <Select
          ...
          options={[
            ...
            ...FLAG_STATUSES.map((s) => ({ value: s, label: s })),
          ]}
          className="w-44"
        />
        <Select
          ...
          options={[
            ...
            ...FLAG_KINDS.map((k) => ({ value: k, label: k })),
          ]}
          className="w-52"
        />
        <Select
          ...
          options={[
            ...
            ...FLAG_SEVERITIES.map((s) => ({ value: s, label: s })),
          ]}
          className="w-44"
        />
      </div>
          ...
          row.status === 'open' && (
            <div className="flex gap-1.5">
              <Button
                size="sm"
                variant="outline"
                onClick={() => setReviewTarget({ row, action: 'reviewed' })}
              >
                Mark reviewed
              </Button>
              <Button
                size="sm"
                variant="outline"
                onClick={() => setReviewTarget({ row, action: 'dismissed' })}
              >
                Dismiss
              </Button>
            </div>
      ...
      <ReasonDialog
        ...
        onConfirm={(reason) =>
          ...
          reviewMutation.mutate({
            ...
            status: reviewTarget.action as FlagStatus & ('reviewed' | 'dismissed'),
            reason,
          })
        }
      />
