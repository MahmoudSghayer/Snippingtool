...
const columns: ColumnDef<AdminSubscriptionListItem, unknown>[] = [
  { accessorKey: 'userEmail', header: 'User' },
  {
    accessorKey: 'plan',
    header: 'Plan',
    cell: (c) => (c.getValue() as AdminSubscriptionListItem['plan']).name,
  },
  {
    accessorKey: 'status',
    header: 'Status',
    cell: (c) => {
      ...
      return (
        <Badge
          tone={
            v === 'active' || v === 'trialing' || v === 'lifetime'
              ? 'positive'
              : v === 'canceled' || v === 'expired'
                ? 'negative'
                : 'warning'
          }
        >
...
export function SubscriptionsAdminPage() {
  ...
  const metricsQuery = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await api.GET('/api/v1/admin/analytics/subscriptions', {
        params: { query: range },
      });
  ...
  const planMix = Object.entries(metricsQuery.data?.planMix ?? {}).map(([key, value], i) => ({
    key,
    label: key,
    value,
    colorIndex: i,
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        title="Subscriptions"
        description="Plan mix, subscription lifecycle metrics, and every account's subscription."
      ...
      <KpiGrid>
        ...
        <StatTile
          ...
          value={items.reduce((sum, i) => sum + i.newSubscriptions, 0)}
        />
        <StatTile
          label="Cancellations"
          value={items.reduce((sum, i) => sum + i.canceledSubscriptions, 0)}
          invertDeltaTone
        />
        <StatTile
          label="Coupon redemptions"
          value={items.reduce((sum, i) => sum + i.couponRedemptions, 0)}
        />
      </KpiGrid>
          ...
          <FormField label="Status" htmlFor="sub-status" className="w-44">
            <Select
              ...
              options={[
                ...
                ...SUBSCRIPTION_STATUSES.map((s) => ({ value: s, label: s })),
              ]}
            />
          </FormField>
        </div>
      ...
      {selected && (
        <Drawer
          ...
          title={selected.userEmail}
          description={`Subscription ${selected.id}`}
        >
