      ...
      switch (event.type) {
        case 'session.revoked': {
          ...
          toast.error('You were signed out', {
            description: event.reason === 'admin_force_logout' ? 'An administrator ended this session.' : 'This session is no longer valid.',
          });
        ...
        case 'subscription.changed': {
          ...
          toast.info('Subscription updated', { description: `Your plan is now ${event.subscription.status}.` });
          break;
        }
        ...
        case 'feature_toggles.changed': {
          ...
          if (isAdmin) toast.info('Feature toggles changed', { description: `${event.toggles.length} toggle(s) updated.` });
          break;
        }
        case 'kill_switch': {
          if (isAdmin) {
            toast.warning(event.active ? 'Kill switch activated' : 'Kill switch deactivated', { description: event.reason });
          }
          break;
        }
