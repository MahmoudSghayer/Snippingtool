/*
 * prices.ts — turning sightings into the three numbers that decide a trade.
 *
 * Ported verbatim from milestone 1's `src/model/prices.js` (types added,
 * logic untouched — see docs/01-architecture.md, project instruction 7:
 * "Price model stays simple statistics... the existing prices.js and its
 * tests are ported, not rewritten"). Everything here is deliberately simple
 * statistics over observed listings. No model fitting, no ML. With a few
 * days of data the percentiles alone beat a crowd-sourced price, and a
 * simple estimator you can reason about is worth more than an
 * accurate-looking one you can't.
 */
...
function percentile(sorted: number[], p: number): number | null {
  ...
  if (loVal === undefined) return null;
  if (lo === hi || hiVal === undefined) return loVal;
  return Math.round(loVal + (hiVal - loVal) * (idx - lo));
}
...
type Verdict = 'still-live' | 'unknown' | 'likely-sold' | 'likely-expired';

function classify(row: AuctionRow, coverageEnd: number, nowMs: number): Verdict {
...
/** Summarise one card. `rows` are auctions from `store/db.ts`'s
 * `auctionsForResource()`. */
export function summarise(rows: AuctionRow[] | null | undefined, nowMs = Date.now()): PriceSummary {
  ...
  return {
    ...
    floor: bins.length ? (bins[0] ?? null) : null,
    p10: percentile(bins, 0.1),
    median: percentile(bins, 0.5),
 ...
 * can pick.
 */
export function margin(summary: PriceSummary | null | undefined, price: number): Margin | null {
...
export function maxSnipePrice(summary: PriceSummary | null | undefined, minProfit: number): number | null {
