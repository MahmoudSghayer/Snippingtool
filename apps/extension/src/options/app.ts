/*
 * options/app.ts — saved filters editor, budgets/governor thresholds
...
function validateField(inputId: string, fieldSchema: FieldSchema, parse: (raw: string) => unknown): boolean {
  const input = byId(inputId) as HTMLInputElement | null;
...
function wireSectionValidation(saveButtonId: string, fields: { id: string; schema: FieldSchema; parse: (raw: string) => unknown }[]): void {
  const button = byId(saveButtonId) as HTMLButtonElement | null;
...
async function render(): Promise<void> {
  ...
  byId('save-targets')?.addEventListener('click', async () => {
    ...
    const minProfitPerSnipe = Number((byId('minProfit') as HTMLInputElement).value);
    const maxCoinsPerSnipe = Number((byId('maxCoinsPerSnipe') as HTMLInputElement).value);
    const sessionCoinBudgetRaw = (byId('sessionCoinBudget') as HTMLInputElement).value;
    try {
  ...
  byId('add-filter')?.addEventListener('click', async () => {
    ...
    const maxPrice = (byId('new-filter-max-price') as HTMLInputElement).value;
