...
async function renderRegister(error?: string): Promise<void> {
  ...
  `);
  byId('register')?.addEventListener('click', onRegisterSubmit);
  byId('back-link')?.addEventListener('click', () => void renderLoggedOut());
}
...
function renderCheckEmail(email: string, notice?: string): void {
  ...
  byId('resend')?.addEventListener('click', async () => {
    const btn = byId('resend') as HTMLButtonElement;
    btn.disabled = true;
    try {
