...
import { getPlatformLifetimeSummary, getPlatformProfitSeries, getProfitLeaderboard, getUserLifetimeProfit, getUserProfitSeries } from '../profits.js';
...
async function insertProfit(userId: string, day: string, values: Partial<{ coinsSpent: number; coinsEarned: number; netProfit: number; snipes: number; successes: number; tradesClosed: number }>) {
...
describe('lib/analytics/profits', () => {
  ...
  it('getPlatformProfitSeries: day granularity is one bucket per calendar day, zero-filled for days with no rows', async () => {
    ...
    await insertProfit(u1, '2024-06-01', { netProfit: 1000, coinsSpent: 500, coinsEarned: 1600, snipes: 4, successes: 2, tradesClosed: 2 });
    await insertProfit(u2, '2024-06-01', { netProfit: 200, coinsSpent: 100, coinsEarned: 320, snipes: 1, successes: 1, tradesClosed: 1 });
    // 2024-06-02 has no rows at all -> must appear zero-filled.
    await insertProfit(u1, '2024-06-03', { netProfit: -50, coinsSpent: 400, coinsEarned: 360, snipes: 3, successes: 0, tradesClosed: 1 });

    const items = await getPlatformProfitSeries(db, { from: '2024-06-01', to: '2024-06-03', granularity: 'day' });
    expect(items).toHaveLength(3);
    expect(items.map((i) => i.bucket)).toEqual(['2024-06-01', '2024-06-02', '2024-06-03']);

    expect(items[0]).toMatchObject({ netProfit: 1200, coinsSpent: 600, coinsEarned: 1920, coinsTraded: 2520, snipes: 5, successes: 3, tradesClosed: 3, activeTraders: 2 });
    expect(items[0]!.snipeSuccessRate).toBeCloseTo(3 / 5, 10);

    expect(items[1]).toMatchObject({ netProfit: 0, coinsSpent: 0, coinsEarned: 0, coinsTraded: 0, snipes: 0, successes: 0, tradesClosed: 0, activeTraders: 0, snipeSuccessRate: 0 });

    expect(items[2]).toMatchObject({ netProfit: -50, coinsSpent: 400, coinsEarned: 360, coinsTraded: 760, snipes: 3, successes: 0, tradesClosed: 1, activeTraders: 1, snipeSuccessRate: 0 });
  });
  ...
  it('getPlatformProfitSeries: week granularity buckets by Monday-anchored ISO week', async () => {
    ...
    const items = await getPlatformProfitSeries(db, { from: '2024-01-01', to: '2024-01-08', granularity: 'week' });
  ...
  it('getPlatformProfitSeries: month granularity buckets by first-of-month', async () => {
    ...
    const items = await getPlatformProfitSeries(db, { from: '2024-02-01', to: '2024-03-01', granularity: 'month' });
  ...
  it('getPlatformProfitSeries: lifetime granularity collapses the whole range into one bucket', async () => {
    ...
    const items = await getPlatformProfitSeries(db, { from: '2024-01-01', to: '2024-12-31', granularity: 'lifetime' });
  ...
  it('getUserProfitSeries: scoped to one user only', async () => {
    ...
    const items = await getUserProfitSeries(db, u1, { from: '2024-04-01', to: '2024-04-01', granularity: 'day' });
    expect(items).toHaveLength(1);
  ...
  it('getProfitLeaderboard: top-N descending and least-N ascending by summed net profit, ties broken by userId', async () => {
    ...
    await insertProfit(uHigh, '2024-08-01', { netProfit: 1000, coinsSpent: 200, coinsEarned: 1200 });
    ...
    const top = await getProfitLeaderboard(db, { from: '2024-08-01', to: '2024-08-01', limit: 2, order: 'top' });
    ...
    const least = await getProfitLeaderboard(db, { from: '2024-08-01', to: '2024-08-01', limit: 2, order: 'least' });
