...
describe('notifications flow', () => {
  ...
  it('lists only the authenticated user notifications, newest first, paginated by cursor', async () => {
    const userA = await createUserSession(
      app,
      'notif-a@example.com',
      'fp-notif-a-0000000000000001',
    );
    const userB = await createUserSession(
      app,
      'notif-b@example.com',
      'fp-notif-b-0000000000000002',
    );
    ...
    const page2 = await app.inject({
      method: 'GET',
      url: `/api/v1/notifications?limit=2&cursor=${page1Body.nextCursor}`,
      headers: bearer(userA.accessToken),
    });
    ...
    const allForA = await app.inject({
      method: 'GET',
  ...
  it('marks a single notification read, scoped to the owner (404 for another user’s notification)', async () => {
    const userA = await createUserSession(
      app,
      'notif-mark-a@example.com',
      'fp-notif-mark-a-000000000001',
    );
    const userB = await createUserSession(
      app,
      'notif-mark-b@example.com',
      'fp-notif-mark-b-000000000002',
    );
    const [rowA] = await seedNotifications(userA.userId, 1);

    const crossOwnerAttempt = await app.inject({
      method: 'POST',
      url: `/api/v1/notifications/${rowA!.id}/read`,
  ...
  it('marks every unread notification read for the authenticated user only', async () => {
    const userA = await createUserSession(
      app,
      'notif-all-a@example.com',
      'fp-notif-all-a-0000000000001',
    );
    const userB = await createUserSession(
      app,
      'notif-all-b@example.com',
      'fp-notif-all-b-0000000000002',
    );
