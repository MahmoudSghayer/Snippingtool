...
```jsonc
{
  ...
  "features": [
    // PLAN_FEATURES[plan] when status is trialing/active/lifetime, else []
    "ledger.recorder",
    "assist.ranker",
  ...
  "license": {
    // the subscription's active license, or null if none has been issued yet
    "keyPrefix": "SL-9F2K",
    "status": "active",
    "maxDevices": 3,
    "expiresAt": null,
...
per-id/per-user _action_ routes below (extend/suspend/cancel/activate/
