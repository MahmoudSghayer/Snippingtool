...
now **Fixed**, each with a dedicated regression test, except row #9, whose
fix is applied but whose specific triggering mechanism (an SW restart
mid-flush) remains unconfirmed against a live Chrome instance in this
session, same as originally filed — see that row's own Status cell.
