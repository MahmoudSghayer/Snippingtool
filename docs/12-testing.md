...
- **A flaky assertion is a bug in the test, not a reason to skip or retry
  ...
  than a fixed `sleep`.
- **A suite whose _subject_ is a race is written to assert the invariant
...
_threshold_ enforced in CI is deliberately not added, since a hard
...
**Why (c) drives `suspend` _and_ `force-logout`, not just `suspend`**: only
...
by `prepare.mjs`, and a _separately built_ extension target
...
an admin-action _ordering_ bug that silently no-ops a WS push — see
...
**Journey (b) passes end to end** (all four cross-app journeys are green
locally under `xvfb-run` and in CI's `e2e-cross-app` job). It took two
fixes, both landed with the CI drive-to-green pass on PR #2:

1. The step "the mock page's passive search was recorded and, once
   flushed, reaches the API" used to report `sent: 0`. The panel's
   "Auctions recorded" row moving off its "—" placeholder was never
   evidence of an observation — `content/index.ts` fills it from the
   boot-time `counts` reply ("0") — and the mock page's own 50ms passive
   search fires before the `document_idle` content script is listening on
   an instantly-fulfilled page. The spec now triggers the search itself via
   the fixture's `window.__mock.triggerPassiveSearch()` once the panel host
   proves the content script is live, and asserts on the counters that only
   move once an observation has crossed into the content script
   ("Searches this session" ≥ 1, then "Auctions recorded" > 0).
   `apps/extension/test/e2e/extension.spec.ts` got the same strengthening.
2. With that real assertion in place, the same step exposed
   [§12 row #10](#12-defects-found): the content script's crash-recovery
   read of `browser.storage.session` throws in a content script, its boot
   aborted, and every later observation crashed on an uninitialised
   binding — no recording, no telemetry. Fixed in `apps/extension/src`
   (state now round-trips through background); the strengthened e2e is the
   live regression test.
3. The final step (admin flips the kill switch → popup reports halted) is
   asserted the way the product actually propagates it: through the
   heartbeat (`license.heartbeat`, the same handler the 10-minute alarm
   fires), which refreshes the cached entitlement the popup renders from.
   The step used to reload the popup _before_ any heartbeat and read the
   deliberately-served fresh cache.
...
(`plugins/rate-limit.ts`'s config-driven limiter is a _second_, independent
...
`auth-login-refresh` originally read from the _same_ pool. Every login
...
it) — which immediately invalidates any _other_ already-issued access token
...
**Status update (full-stack defect-fixing pass, referenced by test file —
no commit hashes, per that pass's own instructions)**: every row below is
now **Fixed**, each with a dedicated regression test. Row #9's fix is
applied, and the journey-(b) failure that was filed alongside it turned out
to be row #10 (found by the CI drive-to-green pass once the e2e assertion
was made real) — see both rows' Status cells.

| #   | Where                                                                                                                                                   | Repro                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Expected                                                                                                                                                                                                                                                                                               | Actual                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Proposed fix                                                                                                                                                                                                                                                                                                                                                                                 | Severity                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `apps/api/src/modules/settings/index.ts` (`PUT /api/v1/settings`)                                                                                       | `TEST_DATABASE_URL=...sniper_ledger_test_qa REDIS_TEST_DB=12 pnpm --filter @sl/api exec vitest run src/test/qa/__tests__/settings-versioning.test.ts` — the `DEFECT:` test, which loops 8 racing pairs to reliably surface it                                                                                                                                                                                                                                          | Two concurrent `PUT /settings` from the same client either both apply (last-write-wins, per the documented "server version wins" sync policy) or the loser gets a clean, documented conflict response                                                                                                  | The handler reads `current.version`, computes `merged.version = current.version + 1` in application code, then inserts into `settings_history` — a unique index on `(user_id, version)`. Two concurrent requests reading the same `current.version` race on that insert; the loser gets an unhandled `500 INTERNAL` (a raw Postgres unique-violation), not a 200 or a documented conflict code                                                                             | Wrap the read-merge-write in a transaction with `SELECT ... FOR UPDATE` on the `user_settings` row (or an `INSERT ... ON CONFLICT DO NOTHING` + retry loop) in `modules/settings/index.ts`'s `PUT` handler                                                                                                                                                                                   | Medium — a genuine race, low real-world likelihood (the same user issuing two concurrent settings writes), but a raw 500 instead of a clean error either way                                                                                                                                                                                                                                                                                                       | **Fixed** — `PUT /settings` now catches the unique-violation and returns `409 CONFLICT`; a related pre-existing race in `getOrCreateUserSettings`'s own first-row insert is fixed the same way. Test: `apps/api/src/test/qa/__tests__/settings-versioning.test.ts` ("concurrent PUTs race...").                                                                                                                                                                                                                                                                                                                                                                              |
| 2   | `packages/shared/src/schemas/activity.ts` (`activityIngestBatchSchema`)                                                                                 | `pnpm --filter @sl/shared exec vitest run test/strictness.test.ts`                                                                                                                                                                                                                                                                                                                                                                                                     | Every batch-ingest envelope schema is `.strict()` (docs/09-security.md's documented mass-assignment convention; every sibling — `reportTradesRequestSchema`, `reportSnipingAttemptsRequestSchema`, `reportFilterStatsRequestSchema`, `telemetryFlushRequestSchema`, `extensionErrorReportSchema` — is) | Was found _not_ `.strict()` (silently stripped an unrecognised top-level key instead of rejecting it) while authoring the strictness sweep above; **fixed during this session** (`.strict()` added to the envelope and its per-event `metadata` objects — see the schema file's own comment)                                                                                                                                                                               | — (already applied)                                                                                                                                                                                                                                                                                                                                                                          | Low — zod's default behaviour strips rather than accepts an unknown key, so no data-integrity/injection risk existed even before the fix; this was a convention/error-message inconsistency, now closed                                                                                                                                                                                                                                                            | **Fixed** (already, at time of filing — no further action this pass).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 3   | `apps/extension/src/popup/main.ts`'s `onRegisterSubmit()` / `apps/extension/src/lib/auth.ts`'s `register()`                                             | Read `apps/extension/src/lib/auth.ts`'s `register()` — it types its response as `LoginResponse` and only persists tokens `if (data.status === 'ok')`; the real `POST /auth/register` response is `{ userId }` (201, no `status` field at all — email verification is required before login works, `docs/03-api.md` §"auth"). `popup/main.ts`'s `onRegisterSubmit()` doesn't check for this — it calls `renderLoggedIn()` unconditionally after any successful register | After "Create an account", the popup either shows a "check your email to verify" state, or falls through to a working login prompt                                                                                                                                                                     | The popup calls `license.bootstrap` with no access token (registration never issued one) and silently fails/shows a broken state — no verification guidance at all                                                                                                                                                                                                                                                                                                         | `popup/main.ts`: after `send('auth.register', ...)`, render a "check your email" state instead of `renderLoggedIn()` (matching what actually happens server-side)                                                                                                                                                                                                                            | Medium — a real first-run UX gap for anyone who registers through the popup instead of the dashboard; this agent's own `tests/e2e/specs/b-extension-bootstrap.spec.ts` works around it by using a pre-verified user and the popup's _login_ form instead (see that spec's header)                                                                                                                                                                                  | **Fixed** — `register()` now returns the real `{userId}` shape (`registerResponseSchema`), the popup renders an explicit "check your email" state with a resend action instead of `renderLoggedIn()`. Test: `apps/extension/test/unit/register-verify-email.test.ts`.                                                                                                                                                                                                                                                                                                                                                                                                        |
| 4   | `apps/extension/src/lib/settings.ts`'s `updateSettings()` vs. `apps/api/src/modules/settings/index.ts`                                                  | `apps/api/src/test/qa/__tests__/settings-versioning.test.ts`'s last test (`'DEFECT: the route the extension patches...'`)                                                                                                                                                                                                                                                                                                                                              | The extension's settings sync uses the same HTTP verb the server exposes                                                                                                                                                                                                                               | `updateSettings()` sends `{ method: 'PATCH' }` to `/api/v1/settings`; the server only registers `app.put(...)` for that path — every real settings sync from the extension 404s                                                                                                                                                                                                                                                                                            | `apps/extension/src/lib/settings.ts`: change `updateSettings()` to send `method: 'PUT'`                                                                                                                                                                                                                                                                                                      | **High** — this breaks the extension's settings sync entirely today, not an edge case                                                                                                                                                                                                                                                                                                                                                                              | **Fixed** — `updateSettings()` now sends `PUT`. Also found (while writing the API-contract test this fix called for) and fixed three more, previously undocumented 404s in `lib/telemetry.ts`'s `flush()` (wrong paths for activity/sniping/trades batch endpoints). Tests: `apps/extension/test/unit/settings-conflict.test.ts`, `apps/api/src/test/qa/__tests__/settings-versioning.test.ts`, `apps/api/src/test/qa/__tests__/extension-api-contract.test.ts`.                                                                                                                                                                                                             |
| 5   | `apps/api/src/modules/auth/service.ts` (`LOGIN_RATE_LIMIT_MAX`/`_WINDOW_MS`)                                                                            | `tests/load` §9 above                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Every rate-limit knob this repo exposes is env-configurable (matching `plugins/rate-limit.ts`'s sibling limiter on the same routes)                                                                                                                                                                    | Hardcoded at `20`/`15 min`, independent of `RATE_LIMIT_LOGIN_MAX`/`_WINDOW_MS`                                                                                                                                                                                                                                                                                                                                                                                             | Read these two from `fastify.config` (already a validated env source) instead of module-level constants                                                                                                                                                                                                                                                                                      | Low (operational, not a security or correctness bug — the limiter does its job; it's just untunable) — documented for anyone load-testing login at realistic scale                                                                                                                                                                                                                                                                                                 | **Fixed** — `login()` now reads `RATE_LIMIT_LOGIN_MAX`/`RATE_LIMIT_LOGIN_WINDOW_MS` via an optional `AuthContext.loginRateLimit` (defaults preserved for callers that omit it). Test: `apps/api/src/test/qa/__tests__/login-rate-limit-configurable.test.ts`.                                                                                                                                                                                                                                                                                                                                                                                                                |
| 6   | `packages/shared`'s `admin/analytics` vs. `trades` granularity enums                                                                                    | `tests/load/scenarios/admin-analytics-overview.js`'s own comment, §9 above                                                                                                                                                                                                                                                                                                                                                                                             | One `granularity` vocabulary across the API for the same concept                                                                                                                                                                                                                                       | `/admin/analytics/*` and `/analytics/me/*` use `'day'\|'week'\|'month'\|'lifetime'`; `/profits` uses `'daily'\|'weekly'\|'monthly'\|'lifetime'`                                                                                                                                                                                                                                                                                                                            | Pick one vocabulary; not attempted here (touches two agents' owned schemas/routes)                                                                                                                                                                                                                                                                                                           | Low — cosmetic/API-design inconsistency, not a bug; only costs an integrator a confused first 400                                                                                                                                                                                                                                                                                                                                                                  | **Fixed** — `/profits` now shares `granularitySchema` (`'day'\|'week'\|'month'\|'lifetime'`) with `/admin/analytics/*`/`/analytics/me/*`; legacy `'daily'/'weekly'/'monthly'` still accepted on input (deprecated alias) for backwards compatibility. Test: `apps/api/src/test/qa/__tests__/granularity-vocabulary.test.ts`.                                                                                                                                                                                                                                                                                                                                                 |
| 7   | `apps/api/src/modules/admin-users/index.ts`'s `suspend`/`force-logout` routes (both call `apps/api/src/modules/auth/repo.ts`'s `revokeAllUserSessions`) | `tests/e2e/specs/c-admin-suspend-ws-audit.spec.ts`'s own header comment — reproduced directly (call `suspend` then `force-logout` on the same target; force-logout returns `200`, but its WS push never arrives) while authoring the journey                                                                                                                                                                                                                           | Calling `force-logout` on an account an admin already suspended still notifies the (already force-logged-out-feeling) user's live session that it's been signed out, or the response says so                                                                                                           | `revokeAllUserSessions` only returns/pushes for sessions matching `WHERE revoked_at IS NULL` at call time; `suspend`'s own earlier `revokeAllUserSessions` call already revoked them, so `force-logout`'s call finds nothing, `publishToUser` is never invoked for any session, and the `200 { ok: true }` response gives no indication anything was skipped                                                                                                               | Either have `force-logout` report `sessionsRevoked: 0` distinctly (it already returns this count in the audit row's `after`, just not in the HTTP response body) so a caller can tell "nothing to revoke" from "revoked and notified", or push a `session.revoked`-shaped notice regardless of whether the DB row was already revoked (best-effort UX signal, not a correctness requirement) | Medium — silent no-op, not a security issue (the account genuinely is signed out either way — suspend's own revocation already achieved that), but the _notification_ one admin action promised silently didn't happen if another already got there first, with no error to catch it                                                                                                                                                                               | **Fixed** — `force-logout` now returns `{ ok, sessionsRevoked, sessionsNotified }` (`sessionsRevoked` can legitimately be 0) and always publishes `session.revoked` for every session id the target has ever had, regardless of that count. Tests: `apps/api/src/test/qa/__tests__/force-logout-after-suspend.test.ts` (real WS socket), `tests/e2e/specs/c-admin-suspend-ws-audit.spec.ts` (now exercises suspend-then-force-logout, the previously-broken order).                                                                                                                                                                                                          |
| 8   | `apps/api/src/modules/payments/webhooks.ts`'s `handleCheckoutCompleted()`                                                                               | `tests/e2e/specs/d-subscription-webhook.spec.ts`'s `DEFECT:` step — reproduced directly with a minimal register→login→hold-token→checkout-webhook→same-token-immediately-401s repro while authoring the journey                                                                                                                                                                                                                                                        | Completing checkout (a server-to-server webhook, nothing the buyer's own open session did) doesn't invalidate the buyer's _own already-issued_ access token                                                                                                                                            | The `stripe_customer_id` backfill (`if (stripeCustomerId) { await tx.update(users).set({ stripeCustomerId })... }`, inside the same transaction that activates the subscription) is a plain `users` UPDATE, which fires the blanket `bump_row_version` trigger — so the access token the buyer was already holding (`ver` = row_version at login time) goes stale the instant the webhook lands, and their very next authenticated request gets `401 AUTH_SESSION_REVOKED` | Don't route the `stripe_customer_id` backfill through a plain `users` UPDATE in this transaction — e.g. restore the pre-checkout `row_version` afterwards (defeats the trigger's purpose elsewhere) or move `stripe_customer_id` off the row-version-covered `users` table                                                                                                                   | Medium — same root-cause pattern as row #1 (a benign, unrelated `users` UPDATE incidentally invalidating a live session via the blanket per-row trigger); in practice masked for most real clients by their own 401→refresh handling (`apps/extension/src/lib/auth.ts`'s `handleUnauthorized`, the dashboard's own API client), so the user-visible cost is one extra silent round trip, not a lockout — but it is real, reproducible, and previously undocumented | **Fixed** — `migrations/0026_users_row_version_exclude_billing.sql` gives `users` its own trigger, `bump_users_row_version()`, which skips the `row_version` bump when the only column that changed is `stripe_customer_id` (an exclude-list, not an allow-list — see that migration's own comment for why: two other existing call sites depend on "any `users` UPDATE bumps row_version" beyond just the named security-relevant columns). Tests: `packages/db/test/triggers.test.ts`, `apps/api/src/test/qa/__tests__/checkout-webhook-token-survives.test.ts` (also confirms password-change still bumps/invalidates), `tests/e2e/specs/d-subscription-webhook.spec.ts`. |

| 9 | `apps/extension/src/lib/telemetry.ts` (`queue`/`enqueue*`/`flush`) | `tests/e2e/specs/b-extension-bootstrap.spec.ts`'s "the mock page's passive search was recorded..." step — consistently reproduced (4+ runs) while authoring this journey: the observation genuinely happens (the panel's own counter moves off its placeholder — polled and confirmed _before_ attempting the flush, ruling out a race with the observation itself), but `telemetry.flush()`, called shortly after via a real popup-login round trip in between, reports `sent: 0` every time | Telemetry queued by the content script survives until the next flush (the 2-minute `chrome.alarms` tick, or a forced flush like this suite's), the same way `lib/auth.ts` persists tokens (`storage.session`/`storage.local`) rather than trusting the service worker to stay alive | `queue` (`let queue = emptyBatches()`) is a bare module-level variable in the service worker's own JS — MV3 service workers are killed for inactivity (Chrome's own ~30s idle timeout) and restarted fresh on the next event with all in-memory state gone; this suite's own popup-login round trip (real network calls, ~15s+) between the observation and the flush attempt is itself enough idle time to plausibly have triggered exactly that. **Not fully confirmed against a live Chrome instance's own internal SW lifecycle events in this session** (no direct visibility into "was the SW actually terminated between these two points" was captured — flagged as the leading, evidence-consistent hypothesis, not a certainty) — but the mechanism (`let queue` with no persistence) is a real, independently-verifiable gap regardless of whether it's what happened in this specific run: any telemetry queued between `chrome.alarms` ticks is silently lost if the SW restarts before the next tick, in real usage, not just in this test | Persist the queue the same way tokens are (e.g. `storage.session`, cleared alongside them, restored on SW startup) instead of a bare module variable — not applied here, `apps/extension/src/**` isn't this suite's file ownership | **High if confirmed** — silent telemetry data loss on a very plausible, everyday MV3 lifecycle event, not an edge case; downgraded to "flag, don't block" pending the confirmation this session's time budget didn't allow (see journey (b)'s own status below) | **Fixed** — the queue now persists to `browser.storage.session` (falling back to `.local`) on every enqueue, with a lazy one-time-per-SW-instance hydration on first use and an additional best-effort flush on `chrome.runtime.onSuspend`. The specific "was the SW actually killed mid-flush in that one journey-(b) run" question is still unconfirmed (as originally filed) — the fix addresses the underlying mechanism regardless, and is proven directly (not inferred) by a test that empties the module registry (`vi.resetModules()`) between an enqueue and a re-import, the closest a Node/jsdom test can get to a real SW restart. Test: `apps/extension/test/unit/telemetry-persistence.test.ts`. Also fixed in the same file: three additional, previously-undocumented wrong flush paths (activity/sniping/trades), found while adding defect #4's API-contract test. **Journey (b) re-run result**: step 6 ("the mock page's passive search was recorded and, once flushed, reaches the API") still does not pass — but bisected directly (`git stash` of every extension-source change, rebuild, re-run against the original pre-fix code) to a **pre-existing, environment-specific root cause independent of this fix**: `reportSearchActivity()`'s `telemetry.enqueue` message never reaches the service worker at all in this exact cross-app harness (confirmed via direct SW-console instrumentation — zero `record`/`telemetry.enqueue`/message-router activity for the whole 20s poll, both before and after this fix), most likely the mock fixture's 50ms auto-triggered passive search (`mock-service-layer.js`) racing the extension's own MAIN-world adapter patch in this specific page-navigation/context setup — `apps/extension/test/e2e/extension.spec.ts`'s own simpler, single-app version of the same mock-fixture flow passes reliably. Not a regression from this fix, and not what row #9 originally flagged (the SW-restart hypothesis). **Closed (CI drive-to-green pass)**: the actual mechanism was row #10 below (the content script's `storage.session` read throwing at boot) plus a weak e2e assertion — see "Journey (b) passes end to end" above; the journey now passes with a stronger assertion. While investigating this, also found and fixed a real bug in this pass's own new code: the popup live-risk-gauge messages (`governor.snapshotPush`/`governor.snapshotGet`) were handled in `background/index.ts` but missing from `backgroundMessageTypeSchema`'s enum, so the envelope validation silently rejected every real one — fixed, regression-tested in `packages/shared/test/ext-messages.test.ts`. |
| 10 | `apps/extension/src/content/index.ts` (crash recovery: `getSession`/`setSession` of `sl.engine.state.v1`) | `pnpm --filter @sl/extension build && xvfb-run -a pnpm --filter @sl/extension test:e2e` with the strengthened "Searches this session ≥ 1 / Auctions recorded > 0" assertions; or load `dist/ledger` unpacked, open the EA page and watch the page console: `Error: Access to storage is not allowed from this context` once at load, then `ReferenceError: Cannot access '<engine binding>' before initialization` on every market response | The M1 recorder works with no account at all (docs/01-architecture.md): every observed market response is trimmed, recorded to IndexedDB and reported as a `search` activity event, whatever happened to the account-gated M2/M3 bootstrap | MV3 content scripts are not a trusted context for `browser.storage.session` (Chrome's default access level is `TRUSTED_CONTEXTS`), so the crash-recovery read threw, `main()` rejected before `governor`/`assist` (declared as `const`/`let` _after_ that `await`) were initialised, and `adapter.onAuctions` → `engineHealthState()` → `governor?.…` hit the temporal dead zone on every observation — after `searches++` but before `reportSearchActivity()`/`record`. The single-app e2e's `not.toBe('—')` assertion could not see it (row #9's note). The shipped `ledger` build recorded nothing and sent no telemetry | Never touch `storage.session` from the content script; persist the governor state via background (which owns `storage.session` already for the risk snapshot); declare the engine bindings before registering adapter callbacks | **Critical** — the listable build's core M1 feature and all product telemetry were dead in every real page load | **Fixed** — `engine.stateGet`/`engine.stateSet` messages (`packages/shared/src/ext-messages.ts`, strict payload schema) handled in `background/governor.ts`; `content/index.ts` no longer imports `lib/storage.ts`, hoists `governor`/`assist`, and reports (never swallows) a failed M2 bootstrap. The `storage.session` access level is deliberately _not_ widened (it also holds the access token). Tests: `apps/extension/test/unit/engine-state-relay.test.ts` (relay round-trip + static "content never imports storage / bindings declared before callbacks" guard), `packages/shared/test/ext-messages.test.ts` (new message types), and the strengthened `apps/extension/test/e2e/extension.spec.ts` + `tests/e2e/specs/b-extension-bootstrap.spec.ts` as the live regression |
