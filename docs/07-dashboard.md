...
/dashboard, /trades, /analytics, /subscriptions, /settings            (any authenticated user)
...
| `/dashboard`, `/trades`, `/analytics`, `/subscriptions`, `/settings`          | `appLayoutRoute.beforeLoad`: session bootstrapped and `status === 'authenticated'`, else redirect to `/login?returnTo=` |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| Page             | Endpoints                                                                                                                                                                     |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/dashboard`     | `GET /analytics/me/overview`, `GET /trades?limit=8`, `GET /devices`, `GET /licenses/me`, `GET /settings`, `GET /risk-events`                                                  |
| `/trades`        | `GET /trades` (cursor-paginated), `POST /trades/:id/close` — the "Record sale" dialog, which previews the EA tax and net before sending and shows what the API computed after |
| `/analytics`     | `GET /analytics/me/profits`, `GET /analytics/me/activity`, `GET /filters`, `GET /filters/stats`                                                                               |
| `/subscriptions` | `GET /plans`, `GET /subscriptions/me`, `POST /payments/checkout`, `GET /licenses/me`                                                                                          |
| `/settings`      | `GET/PUT /settings`, `PATCH /users/me`, `POST /auth/password/change`, `POST /auth/totp/*`, `GET/DELETE /devices`                                                              |
