...
Every phase below has been built on `claude/happy-knuth-850cu9` and verified
with the repo-wide checks (`pnpm typecheck && pnpm lint && pnpm build && pnpm
test`: 683 tests green across seven packages, both extension builds, the
listable build free of automation code). What remains is the go-live work
