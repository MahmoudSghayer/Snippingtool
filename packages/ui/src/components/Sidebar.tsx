...
export interface SidebarProps {
  ...
  linkComponent?: (props: { href: string; className: string; children: ReactNode; 'aria-current'?: 'page' }) => ReactNode;
  className?: string;
}

export function Sidebar({ brand, sections, footer, linkComponent, className }: SidebarProps) {
  const Link = linkComponent ?? (({ href, className: c, children, ...rest }) => (
  ...
  return (
    <aside className={cn('flex h-full w-64 shrink-0 flex-col border-r border-[--sl-border] bg-[--sl-surface]', className)}>
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4">
        {sections.map((section, i) => (
          <div key={section.title ?? i} className={i > 0 ? 'mt-5' : undefined}>
            {section.title && (
              <p className="mb-1.5 px-2 text-xs font-semibold uppercase tracking-wide text-[--sl-fg-muted]">{section.title}</p>
            )}
