...
import {
  ...
  updateFeatureToggleRequestSchema,
...
describe('strictness: unknown top-level keys are rejected (mass-assignment defence)', () => {
  it.each([
    ...
    [
      'loginRequestSchema',
      loginRequestSchema,
      { email: 'a@example.com', password: 'x', device: device() },
    ],
    ...
    [
      'passwordResetConfirmSchema',
      passwordResetConfirmSchema,
      { token: 't', password: 'correcthorse1' },
    ],
    ...
    [
      'bootstrapRequestSchema',
      bootstrapRequestSchema,
      { device: device(), extensionVersion: '1.0.0', buildTarget: 'ledger' },
    ],
    ...
    [
      ...
      {
        events: [
          { type: 'heartbeat', occurredAt: isoNow(), metadata: { extensionVersion: '1.0.0' } },
        ],
      },
    ],
    ...
    expect(
      valid.success,
      `base fixture should itself be valid: ${JSON.stringify(valid.success ? {} : valid.error.issues)}`,
    ).toBe(true);
  ...
  it('updateUserSettingsRequestSchema rejects an unrecognised key nested inside a partial section', () => {
    const result = updateUserSettingsRequestSchema.safeParse({
      targets: { minProfitPerSnipe: 1000, madeUpField: true },
    });
    expect(result.success).toBe(false);
  });
...
describe('strictness: batch envelopes reject an oversize array', () => {
  it('reportTradesRequestSchema caps trades at 200', () => {
    ...
    expect(reportTradesRequestSchema.safeParse({ trades: Array(200).fill(one) }).success).toBe(
      true,
    );
    expect(reportTradesRequestSchema.safeParse({ trades: Array(201).fill(one) }).success).toBe(
      false,
    );
  ...
  it('reportSnipingAttemptsRequestSchema caps attempts at 200', () => {
    ...
    expect(
      reportSnipingAttemptsRequestSchema.safeParse({ attempts: Array(200).fill(one) }).success,
    ).toBe(true);
    expect(
      reportSnipingAttemptsRequestSchema.safeParse({ attempts: Array(201).fill(one) }).success,
    ).toBe(false);
  });
  ...
  it('reportFilterStatsRequestSchema caps stats at 200', () => {
    ...
    expect(reportFilterStatsRequestSchema.safeParse({ stats: Array(200).fill(one) }).success).toBe(
      true,
    );
    expect(reportFilterStatsRequestSchema.safeParse({ stats: Array(201).fill(one) }).success).toBe(
      false,
    );
  });
  ...
  it('telemetryFlushRequestSchema caps events at 500', () => {
    ...
    expect(
      telemetryFlushRequestSchema.safeParse({ ...base, events: Array(500).fill(one) }).success,
    ).toBe(true);
    expect(
      telemetryFlushRequestSchema.safeParse({ ...base, events: Array(501).fill(one) }).success,
    ).toBe(false);
  });
  ...
  it('extensionErrorReportSchema caps errors at 100 and rejects an oversize message/stack', () => {
    ...
    expect(
      extensionErrorReportSchema.safeParse({ ...base, errors: Array(100).fill(one) }).success,
    ).toBe(true);
    expect(
      extensionErrorReportSchema.safeParse({ ...base, errors: Array(101).fill(one) }).success,
    ).toBe(false);

    expect(
      extensionErrorReportSchema.safeParse({
        ...base,
        errors: [{ message: 'x'.repeat(2000), occurredAt: isoNow() }],
      }).success,
    ).toBe(true);
    expect(
      extensionErrorReportSchema.safeParse({
        ...base,
        errors: [{ message: 'x'.repeat(2001), occurredAt: isoNow() }],
      }).success,
    ).toBe(false);
    expect(
      extensionErrorReportSchema.safeParse({
        ...base,
        errors: [{ message: 'x', stack: 'y'.repeat(8001), occurredAt: isoNow() }],
      }).success,
    ).toBe(false);
  });
  ...
  it('activityIngestBatchSchema caps events at 500', () => {
    ...
    expect(activityIngestBatchSchema.safeParse({ events: Array(500).fill(one) }).success).toBe(
      true,
    );
    expect(activityIngestBatchSchema.safeParse({ events: Array(501).fill(one) }).success).toBe(
      false,
    );
  });
});
...
describe('strictness: oversize scalar fields are rejected at their documented .max()', () => {
  it("governorSettingsSchema rejects a value above each field's absolute ceiling", () => {
    ...
    expect(governorSettingsSchema.safeParse(valid).success).toBe(true);
    expect(
      governorSettingsSchema.safeParse({
        ...valid,
    ...
    expect(
      governorSettingsSchema.safeParse({
        ...valid,
        sessionLengthMinutes: GOVERNOR_ABSOLUTE_LIMITS.sessionLengthMinutes.max + 1,
      }).success,
    ).toBe(false);
    expect(
      governorSettingsSchema.safeParse({
        ...valid,
        maxCoinFlowPerHour: GOVERNOR_ABSOLUTE_LIMITS.maxCoinFlowPerHour.max + 1,
      }).success,
    ).toBe(false);
  });
  ...
  it('createSavedFilterRequestSchema rejects a name over 80 chars', () => {
    expect(
      createSavedFilterRequestSchema.safeParse({ name: 'x'.repeat(80), filter: {} }).success,
    ).toBe(true);
    expect(
      createSavedFilterRequestSchema.safeParse({ name: 'x'.repeat(81), filter: {} }).success,
    ).toBe(false);
  });

  it('adminSuspendUserRequestSchema rejects a reason over 1000 chars', () => {
    expect(adminSuspendUserRequestSchema.safeParse({ reason: 'x'.repeat(1000) }).success).toBe(
      true,
    );
    expect(adminSuspendUserRequestSchema.safeParse({ reason: 'x'.repeat(1001) }).success).toBe(
      false,
    );
  });
  ...
  it('deviceFingerprintSchema rejects an oversize fingerprint/name', () => {
    ...
    expect(
      deviceFingerprintSchema.safeParse({ fingerprint: 'a'.repeat(20), name: 'x'.repeat(121) })
        .success,
    ).toBe(false);
  });
  ...
  it('searchActivitySchema rejects an oversize filterHash', () => {
    const valid = {
      ...
      occurredAt: isoNow(),
      metadata: { filterHash: 'a'.repeat(128), resultsCount: 0 },
    };
    expect(searchActivitySchema.safeParse(valid).success).toBe(true);
    expect(
      searchActivitySchema.safeParse({
        ...valid,
        metadata: { ...valid.metadata, filterHash: 'a'.repeat(129) },
      }).success,
    ).toBe(false);
  });
});
