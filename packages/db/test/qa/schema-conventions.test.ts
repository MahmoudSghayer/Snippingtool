...
const EXCEPTIONS: Record<string, { missing: Column[]; reason: string }> = {
  ...
  admin_actions: {
    ...
    reason: 'write-once admin activity log (§6.1)',
  },
  ...
  email_verifications: {
    missing: ['updated_at', 'deleted_at', 'row_version'],
    reason: 'ephemeral single-use token (§6.2)',
  },
  password_resets: {
    missing: ['updated_at', 'deleted_at', 'row_version'],
    reason: 'ephemeral single-use token (§6.2)',
  ...
  coupon_redemptions: {
    missing: ['updated_at', 'deleted_at', 'row_version'],
    reason: 'append-only redemption record (§6.4)',
  },
  payment_history: {
    missing: ['updated_at', 'deleted_at', 'row_version'],
  ...
  payments: {
    missing: ['deleted_at'],
    reason: 'financial record, protected by RESTRICT rather than soft delete (§6.4)',
  },
  // §6.5 partitioned write-once telemetry: has created_at alongside occurred_at, no updated_at/deleted_at/row_version
  user_activity: {
    missing: ['updated_at', 'deleted_at', 'row_version'],
    reason: 'partitioned write-once telemetry, keyed on occurred_at (§6.5)',
  },
  search_activity: {
    missing: ['updated_at', 'deleted_at', 'row_version'],
    reason: 'partitioned write-once telemetry, keyed on occurred_at (§6.5)',
  ...
  risk_budget_events: {
    missing: ['updated_at', 'deleted_at', 'row_version'],
    reason: 'governor decision log, write-once (§6.5)',
  },
  // §6.6 profits/filter_stats/saved_filters-adjacent rollups — not individually soft-deleted
  profits: {
    missing: ['deleted_at'],
    reason: 'daily rollup upserted by profits.rollup, not a user-owned deletable entity (§6.6)',
  },
  filter_stats: {
    missing: ['deleted_at'],
    reason: 'ranker scoring history rollup, not individually soft-deleted (§6.6)',
  },
  // §6.7 settings/notifications
  user_settings: {
    missing: ['deleted_at'],
    reason:
      'one row per user, cascades with the user rather than being soft-deleted independently (§6.7)',
  },
  settings_history: {
    missing: ['updated_at', 'deleted_at', 'row_version'],
    reason: 'append-only snapshot on every settings change (§6.7)',
  },
  notifications: {
    missing: ['deleted_at'],
    reason: 'read_at is the terminal state a notification reaches, not soft-delete (§6.7)',
  },
  ...
  flags: {
    missing: ['deleted_at'],
    reason: 'evidence record; reviewed_at/status is the terminal state, not soft-delete (§6.8)',
  },
  // §6.9 audit_logs — append-only, tamper-resistant (REVOKE UPDATE/DELETE + trigger)
  audit_logs: {
    missing: ['updated_at', 'deleted_at', 'row_version'],
    reason: 'append-only, tamper-resistant audit trail, keyed on occurred_at (§6.9)',
  },
  // §6.10 system/analytics — config and generated metrics, not soft-deletable entities
  feature_toggles: {
    missing: ['deleted_at'],
    reason: 'toggle is enabled/disabled in place, not soft-deleted (§6.10)',
  },
  system_config: {
    missing: ['deleted_at'],
    reason: 'config key is updated in place, not soft-deleted (§6.10)',
  },
  ip_activity: {
    missing: ['deleted_at'],
    reason: 'rolling per-(ip,user) counters, not a soft-deletable entity (§6.10)',
  },
  extension_installs: {
    missing: ['deleted_at'],
    reason: 'uninstalled_at is the terminal state (§6.10)',
  },
  analytics_daily: {
    missing: ['deleted_at'],
...
describe('schema conventions: created_at/updated_at/deleted_at/row_version per docs/02-database.md', () => {
  ...
  it('every table has exactly the standard audit columns the docs say it should', async () => {
    ...
    expect(byTable.size, 'expected to find at least one table with audit columns').toBeGreaterThan(
      0,
    );
