...
const QA_DATABASE_URL = process.env.QA_TEST_DATABASE_URL ?? 'postgres://sl:sl@127.0.0.1:5432/sniper_ledger_test_qa';
...
const SPOT_CHECK_TABLES = ['users', 'sessions', 'subscriptions', 'trades', 'audit_logs', 'analytics_daily'];
...
describe('migration reversibility (down applies in reverse, then re-applies cleanly)', () => {
  ...
  it('rolls back every migration but 0001 (cluster-global roles/extensions) one at a time, in reverse order', async () => {
    for (let i = allFiles.length - 1; i >= 1; i--) {
      ...
      const before = await sql<{ filename: string }[]>`SELECT filename FROM schema_migrations ORDER BY filename DESC LIMIT 1`;
      expect(before[0]?.filename, `expected ${expectedFile} to be the most-recently-applied migration before rolling it back`).toBe(expectedFile);
    ...
    const remainingFiles = await sql<{ filename: string }[]>`SELECT filename FROM schema_migrations ORDER BY filename`;
    ...
    const remainingTableNames = tables.map((t) => t.tablename).filter((name) => name !== 'schema_migrations');
    expect(remainingTableNames, `expected no application tables left after rolling back every migration but 0001, found: ${remainingTableNames.join(', ')}`).toEqual([]);
  });
