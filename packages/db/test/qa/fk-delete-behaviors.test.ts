...
const EXPECTED: ExpectedFk[] = [
  // §6.1
  { table: 'admin_users', column: 'user_id', onDelete: 'RESTRICT', doc: "can't hard-delete a user who's still an admin" },
  { table: 'admin_actions', column: 'admin_user_id', onDelete: 'RESTRICT', doc: 'admin activity log must outlive a removed admin_users row' },
  // §6.2
  { table: 'devices', column: 'user_id', onDelete: 'CASCADE', doc: 'pure child of users' },
  { table: 'devices', column: 'license_id', onDelete: 'SET NULL', doc: 'optional pointer to the currently-validating license' },
  { table: 'sessions', column: 'user_id', onDelete: 'CASCADE', doc: 'pure child of users' },
  { table: 'sessions', column: 'device_id', onDelete: 'SET NULL', doc: 'device removal must not cascade-delete sessions' },
  { table: 'email_verifications', column: 'user_id', onDelete: 'CASCADE', doc: 'ephemeral token, pure child of users' },
  // §6.3
  { table: 'subscriptions', column: 'user_id', onDelete: 'RESTRICT', doc: 'entitlement record never silently orphaned' },
  { table: 'subscriptions', column: 'plan_id', onDelete: 'RESTRICT', doc: 'entitlement record never silently orphaned' },
  { table: 'subscriptions', column: 'granted_by_admin_id', onDelete: 'SET NULL', doc: 'optional actor' },
  { table: 'licenses', column: 'subscription_id', onDelete: 'RESTRICT', doc: 'financial/entitlement record' },
  { table: 'licenses', column: 'user_id', onDelete: 'RESTRICT', doc: 'financial/entitlement record' },
  // §6.4
  { table: 'coupon_redemptions', column: 'coupon_id', onDelete: 'RESTRICT', doc: 'financial record: RESTRICT on coupon_id/user_id' },
  { table: 'coupon_redemptions', column: 'user_id', onDelete: 'RESTRICT', doc: 'financial record: RESTRICT on coupon_id/user_id' },
  { table: 'coupon_redemptions', column: 'subscription_id', onDelete: 'SET NULL', doc: 'redemption event itself must be retained' },
  ...
  { table: 'payments', column: 'coupon_id', onDelete: 'SET NULL', doc: 'payment amount already reflects the discount' },
  { table: 'payment_history', column: 'payment_id', onDelete: 'CASCADE', doc: 'pure child of payments' },
  // §6.5 — telemetry cascades (supports GDPR erasure)
  { table: 'user_activity', column: 'user_id', onDelete: 'CASCADE', doc: 'pure child of users, supports GDPR erasure' },
  { table: 'user_activity', column: 'device_id', onDelete: 'SET NULL', doc: 'device removal must not cascade-delete activity' },
  { table: 'search_activity', column: 'user_id', onDelete: 'CASCADE', doc: 'pure child of users, supports GDPR erasure' },
  { table: 'sniping_activity', column: 'user_id', onDelete: 'CASCADE', doc: 'pure child of users, supports GDPR erasure' },
  { table: 'risk_budget_events', column: 'user_id', onDelete: 'CASCADE', doc: 'pure child telemetry' },
  { table: 'risk_budget_events', column: 'device_id', onDelete: 'SET NULL', doc: 'optional pointer' },
  { table: 'risk_budget_events', column: 'session_id', onDelete: 'SET NULL', doc: 'optional pointer' },
  // §6.6
  ...
  { table: 'filter_stats', column: 'filter_id', onDelete: 'CASCADE', doc: 'pure child of saved_filters' },
  // §6.7
  ...
  { table: 'bans', column: 'user_id', onDelete: 'SET NULL', doc: 'an IP/device/hwid ban must survive account deletion' },
  { table: 'bans', column: 'issued_by', onDelete: 'SET NULL', doc: 'optional actor' },
  { table: 'flags', column: 'user_id', onDelete: 'RESTRICT', doc: 'a flag is evidence, must not be silently lost' },
  { table: 'flags', column: 'reviewed_by', onDelete: 'SET NULL', doc: 'optional actor' },
  // §6.10
  { table: 'ip_activity', column: 'user_id', onDelete: 'SET NULL', doc: 'rolling counter survives account deletion' },
  { table: 'extension_installs', column: 'user_id', onDelete: 'SET NULL', doc: 'nullable — pre-login installs' },
];
