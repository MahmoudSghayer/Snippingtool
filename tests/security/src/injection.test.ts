...
import { bearer, buildTestApp, createUserSession, TEST_PASSWORD, nextIp, type TestApp } from './helpers.js';
...
const XSS_PAYLOADS = [`<script>alert(document.cookie)</script>`, `<img src=x onerror=alert(1)>`, `"><svg onload=alert(1)>`];
...
describe('injection payload suite', () => {
  ...
  it.each(SQLI_PAYLOADS)('SQLi payload %s in a saved filter name: stored and returned verbatim, never breaks the query (proves parameterisation)', async (payload) => {
    const user = await createUserSession(app, `sqli-${SQLI_PAYLOADS.indexOf(payload)}@example.com`, `sqli-fp-${SQLI_PAYLOADS.indexOf(payload)}-000000001`);
    ...
    const listRes = await app.inject({ method: 'GET', url: '/api/v1/filters', headers: bearer(user.accessToken) });
    expect(listRes.statusCode).toBe(200);
    expect((listRes.json() as Array<{ name: string }>).some((f) => f.name === payload)).toBe(true);
  });

  it.each(XSS_PAYLOADS)('XSS payload %s in a saved filter name: stored and returned as inert JSON data, never executed server-side, never 500s', async (payload) => {
    const idx = XSS_PAYLOADS.indexOf(payload);
    const user = await createUserSession(app, `xss-${idx}@example.com`, `xss-fp-${idx}-00000000000001`);
  ...
  it.each(SQLI_PAYLOADS)('SQLi payload %s as a login email: rejected by schema validation (400), never reaches the query, never 500s', async (payload) => {
    const res = await app.inject({
      method: 'POST',
      url: '/api/v1/auth/login',
      remoteAddress: nextIp(),
      payload: { email: payload, password: TEST_PASSWORD, device: { fingerprint: 'x'.repeat(20) } },
    });
    expect(res.statusCode).toBe(400);
    expect(res.json().code).toBe('VALIDATION_FAILED');
  });

  it.each(NOSQL_PAYLOADS)('NoSQL/object-injection-style payload as a login password: rejected by schema validation (400), never 500s (type confusion never reaches argon2/Drizzle)', async (payload) => {
    const res = await app.inject({
      method: 'POST',
      url: '/api/v1/auth/login',
      remoteAddress: nextIp(),
      payload: { email: 'nosql-target@example.com', password: payload, device: { fingerprint: 'x'.repeat(20) } },
    });
    expect(res.statusCode).toBe(400);
  });
  ...
  it('a SQLi payload as a pagination cursor never reaches SQL: decodeCursor() treats it as undecodable and the route falls back to the first page, never a 500', async () => {
    const user = await createUserSession(app, 'sqli-cursor@example.com', 'sqli-cursor-fp-0000000000001');
    const res = await app.inject({
      method: 'GET',
  ...
  it('a path-traversal / non-UUID payload as a :id param is rejected by param validation (400), never reaches the query', async () => {
    const user = await createUserSession(app, 'traversal@example.com', 'traversal-fp-0000000000001');
    const res = await app.inject({
      method: 'DELETE',
