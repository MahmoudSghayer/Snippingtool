...
import { bearer, createUserSession, device, nextIp, TEST_PASSWORD, buildTestApp, type TestApp } from './helpers.js';
...
describe('CSRF (docs/04-auth.md §10)', () => {
  ...
  it('sets sl_at httpOnly + sl_csrf (not httpOnly, so the dashboard\'s JS can read it) on login', async () => {
    const { cookies } = await loginAsDashboard('csrf-cookies@example.com', 'csrf-fp-cookies-00000000000');
  ...
  it('rejects a cookie-session mutation with no x-csrf-token header (403)', async () => {
    const { cookies } = await loginAsDashboard('csrf-missing@example.com', 'csrf-fp-missing-00000000000');
  ...
  it('rejects a cookie-session mutation with a wrong/forged x-csrf-token (403)', async () => {
    const { cookies } = await loginAsDashboard('csrf-forged@example.com', 'csrf-fp-forged-000000000000');
    const res = await app.inject({
      method: 'POST',
      url: '/api/v1/auth/password/change',
      cookies: { sl_at: cookies.sl_at!, sl_csrf: cookies.sl_csrf! },
  ...
  it('accepts a cookie-session mutation with the correct x-csrf-token (matching the sl_csrf cookie)', async () => {
    ...
    expect(res.statusCode, `expected CSRF to pass with the cookie's own plaintext token, got ${res.statusCode}: ${res.body}`).toBe(200);
  });

  it('a bearer-authenticated mutation needs no CSRF token at all (exempt by construction)', async () => {
    const session = await createUserSession(app, 'csrf-bearer@example.com', 'csrf-fp-bearer-00000000000');
    const res = await app.inject({
      method: 'PATCH',
      url: '/api/v1/users/me',
  ...
  it('a GET under a cookie session never requires a CSRF token', async () => {
    const { cookies } = await loginAsDashboard('csrf-get@example.com', 'csrf-fp-get-00000000000000');
    const res = await app.inject({ method: 'GET', url: '/api/v1/users/me', cookies: { sl_at: cookies.sl_at! } });
    expect(res.statusCode).toBe(200);
  });
});
